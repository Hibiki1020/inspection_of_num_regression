...
class Trainer:
            ...
            for phase in ["train", "valid"]:
                ...
                for img_deg, label_deg, img_rad, label_rad in tqdm(self.dataloaders_dict[phase]):
                    ...
                    with torch.set_grad_enabled(phase == "train"):  #compute grad only in "train"
                        ...
                        ## add loss
                        epoch_loss_degree += loss_deg.item() * label_deg.size(0)
                        epoch_loss_radian += loss_rad.item() * label_rad.size(0)

                ## average loss
                epoch_loss_degree = epoch_loss_degree / len(self.dataloaders_dict[phase].dataset)
                print("{} Loss: {:.4f}".format(phase, epoch_loss_degree))
                ...
                ## record
                if phase == "train":
                    record_train_loss_deg.append(epoch_loss_degree)
                    writer.add_scalar("Loss/train", epoch_loss_degree, epoch)

                    record_train_loss_rad.append(epoch_loss_radian)
                    writer.add_scalar("Loss/train", epoch_loss_radian, epoch)
                    
                else:
                    record_valid_loss_deg.append(epoch_loss_degree)
                    writer.add_scalar("Loss/train", epoch_loss_degree, epoch)

                    record_valid_loss_rad.append(epoch_loss_radian)
                    writer.add_scalar("Loss/train", epoch_loss_radian, epoch)

            if record_train_loss_deg and record_valid_loss_deg:
                writer.add_scalars("Loss/train_and_val", {"train": record_train_loss_deg[-1], "valid": record_valid_loss_deg[-1]}, epoch)
    ...
    def saveWeight_Interval(self, epoch):
        save_path_deg = self.save_top_path + "/weights" + "_" + str(epoch) + "_deg.pth"
        save_path_rad = self.save_top_path + "/weights" + "_" + str(epoch) + "_rad.pth"
        torch.save(self.net_degree.state_dict(), save_path_deg)
        torch.save(self.net_radian.state_dict(), save_path_rad)
        print("Saved Weight in Epoch: {}".format(epoch))

    def saveParam(self):
        save_path_deg = self.save_top_path + "/param_deg.pth"
        save_path_rad = self.save_top_path + "/param_rad.pth"
        torch.save(self.net_degree.state_dict(), save_path_deg)
        torch.save(self.net_radian.state_dict(), save_path_rad)
        print("Saved Param")

    def saveGraph(self, record_train_loss_deg, record_valid_loss_deg, record_train_loss_rad, record_valid_loss_rad):
        fig = plt.figure()
        ax1 = fig.add_subplot(1, 2, 1)
        ax2 = fig.add_subplot(1, 2, 2)
        
        ax1.plot(range(len(record_train_loss_deg)), record_train_loss_deg, label = "train_loss_deg")
        ax1.plot(range(len(record_valid_loss_deg)), record_valid_loss_deg, label = "valid_loss_deg")

        ax1.plot(range(len(record_train_loss_rad)), record_train_loss_rad, label = "train_loss_rad")
        ax1.plot(range(len(record_valid_loss_rad)), record_valid_loss_rad, label = "valid_loss_rad")

        plt.savefig(self.save_top_path + "/loss_graph.png")
        plt.show()
        plt.close()
...
if __name__ == "__main__":
    ...
    print("Load Network")
    net_degree = network_mod.Network(resize, dropout_rate)
