...
class FrameInfer:
    def __init__(self, CFG):
        ...
        self.degree_infer_log_file_name = self.CFG["degree_infer_log_file_name"]
        ...
        self.net_degree = self.getNetwork(self.degree_weights_path)
    ...
    def spin(self):
        ...
        for img_deg, label_deg, img_rad, label_rad in self.test_dataset:
            start_clock = time.time()
            
            img_deg = img_deg.unsqueeze(dim=0)
            ...
            result_csv_degree.append(tmp_result_deg)
        ...
        return result_csv_degree, result_csv_radian

    def save_csv(self, result_csv_degree, result_csv_radian):
        result_csv_path_degree = os.path.join(self.infer_log_top_directory, self.degree_infer_log_file_name)
        result_csv_path_radian = os.path.join(self.infer_log_top_directory, self.radian_infer_log_file_name)

        csv_file = open(result_csv_path_degree, 'w')
        csv_w = csv.writer(csv_file)
        for row in result_csv_degree:
            csv_w.writerow(row)
        csv_file.close()
        print("Save Inference Data in Degree")
        print(result_csv_path_degree)
...
if __name__ == '__main__':
    ...
    result_csv_degree, result_csv_radian = frame_infer.spin()
    frame_infer.save_csv(result_csv_degree, result_csv_radian)